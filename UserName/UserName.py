"""
http://community.topcoder.com/stat?c=problem_statement&pm=2913
Problem Statement for UserName
...
class UserName(object):
    ...
    def newMember(existingNames, newName):
        ...
        def validateName(newName):
            """ (bool) Ensures newName meets constraints """
            for char in newName:
                if not char.isalpha(): return False
            return True

        if not validateName(newName):
            print "Invalid name: %s" % newName
            return None
...
def main():
    ...
    tests = [(["MasterOfDisaster", "DingBat", "Orpheus", "WolfMan", "MrKnowItAll"], "TygerTyger"),
