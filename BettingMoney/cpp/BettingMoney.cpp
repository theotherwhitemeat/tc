...
class BettingMoney{
    public:
        int moneyMade(vector<int> amounts,
                      vector<int> centsPerDollar,
                      int finalResult);
};

int BettingMoney::moneyMade(vector<int> amounts,
                      vector<int> centsPerDollar,
                      int finalResult){

    int total_income = 0;
    int total_payout = 0;
    // loop through arrays, add up the take
    for (int i=0; i < amounts.size(); i++) {
        if (i == finalResult){
            total_payout += centsPerDollar[i] * amounts[i]; 
        }
        else{
            total_income += amounts[i];
        }
    }
    // the return value here is in cents, so we do a conversion
    return (total_income*100 - total_payout);
}
...
class BettingMoney(object):
    ...
    def moneyMade(amounts, centsPerDollar, finalResult):
        """ (int) Determines the net income for a given set of bets, and returns
            the value in cents """

        totalIncome = 0
        totalCost = amounts[finalResult] * centsPerDollar[finalResult]
        # Calculate the income
        for i, item in enumerate(amounts):
            if i != finalResult:
                totalIncome += item

        # Convert income dollars to cents, and subtract the winnings from income
        return (totalIncome * 100) - totalCost


def main():
    """ sup main """

    tests = [([10, 20, 30], [20, 30, 40], 1,),
             ([200, 300, 100], [10, 10, 10], 2),
             ([100, 100, 100, 100], [5, 5, 5, 5], 0),
             ([5000, 5000], [100, 2], 0),
             ([100], [10], 0)]
    ...
    for item in tests:
        print "%s resulted in profit of: %d" % (item, BettingMoney.moneyMade(*item))
